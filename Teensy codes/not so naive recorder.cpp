...
#define Buttonpin1 36 //just for testing will be replaced by bluetooth
Bounce2::Button button1 = Bounce2::Button(); //just for testing will be replaced by bluetooth


void setup() {

    button1.attach(Buttonpin1, INPUT); //just for testing will be replaced by bluetooth
    button1.interval(15); //just for testing will be replaced by bluetooth
    ...
    // add set up code to receive from vluettoth here :: aka set the mic the record, the mic gain, the name of the files to record to on the SD card 
...
void loop() {
    ...
    if (recording == false){
        //enter setup mode so that we can start a new recording, the last line in this if statement should be startRecording(); 
        //which turns recording to true.
    }  
}
